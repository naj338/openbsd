...
# Build on top of the packaged torrc rather than replacing it.  The
# package sets DataDirectory (/var/tor) and logging; drop those and tor
# falls back to ~/.tor, which it cannot write once it has dropped to
# _tor, and it exits at start-up.
if [ -f "$TORRC.pretor" ]; then
	sed -e '/^[[:space:]]*SocksPort/d' \
		-e '/^[[:space:]]*HTTPTunnelPort/d' \
		-e '/^[[:space:]]*DNSPort/d' \
		-e '/^[[:space:]]*AutomapHosts/d' \
		-e '/^[[:space:]]*VirtualAddrNetworkIPv4/d' \
...
if ! grep -q '^[[:space:]]*DataDirectory' "$TORRC"; then
	echo "DataDirectory /var/tor" >>"$TORRC"
fi

cat >>"$TORRC" <<EOF

# --- openbsd-torify.sh -----------------------------------------------
# Started as root so DNSPort can bind 53, then drops to _tor at once.
User _tor
...
# The system resolver points here; names get resolved by Tor, not the ISP.
...
msg "checking the tor configuration"
if ! tor --verify-config -f "$TORRC" >/tmp/torcheck.$$ 2>&1; then
	warn "tor rejected the configuration:"
	tail -n 15 /tmp/torcheck.$$ >&2
	rm -f /tmp/torcheck.$$
	die "not going any further; nothing else has been changed"
fi
rm -f /tmp/torcheck.$$

# rcctl will not set variables on a service that is not enabled yet.
msg "enabling tor"
rcctl enable tor
...
# Start Tor and make sure it stays up BEFORE touching pf, resolv.conf or
# the profile.  A failure here should leave the machine exactly as it was.
msg "starting tor"
if ! rcctl restart tor; then
	warn "tor would not start on port $DNS_PORT; falling back to 9053"
	sed -i "s|^DNSPort .*|DNSPort 127.0.0.1:9053|" "$TORRC"
	DNS_PORT="9053"
	rcctl set tor user '' 2>/dev/null || true
	if ! rcctl restart tor; then
		warn "tor still will not start. Last lines of the daemon log:"
		tail -n 20 /var/log/daemon >&2 2>/dev/null || true
		die "nothing else has been changed"
	fi
fi
msg "tor is running"
...
if netstat -an -f inet 2>/dev/null | grep -q "127\.0\.0\.1\.$DNS_PORT"; then
	...
	save "$RESOLV"
	msg "pointing $RESOLV at Tor's DNS port"
	cat >"$RESOLV" <<EOF
...
EOF
	...
	warn "Tor's DNS port is not on 53, so $RESOLV is left alone."
	warn "  System name lookups will simply fail, which is safe -- pf"
	warn "  blocks them anyway. Use torsocks, which resolves through Tor,"
	warn "  or a browser set to SOCKS with remote DNS."
fi
